...
- All 500 bulk-created guides are intentionally `draft`, `noindex`, and `evidenceStatus: brief`. They are not client-readable under the supplied rules.
- The eight in-app editorial samples are local, noindex previews that demonstrate the final page system.
...
5. Re-run the production app and confirm that an unsigned visitor cannot read draft guide or discovery documents, while a `published` test record appears in search and at its canonical route.
...
After publishing, rebuild Vercel so the sitemap and prerendered SEO pages include the newly indexable URLs.
