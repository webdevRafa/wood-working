...
const pages: Record<StaticPageKey, { eyebrow: string; title: string; description: string; sections: Array<{ heading: string; body: string[] }> }> = {
  about: {
    ...
    sections: [
      { heading: 'What we publish', body: ['The 500-guide library covers beginner skills, project planning, materials, shop setup, troubleshooting, and tool decisions. Every current guide passed the publication checks for evidence, title fulfillment, safety context, natural language, and a complete reader outcome.', 'Research-reviewed does not mean hands-on tested. Exact product specifications, finish schedules, hardware geometry, and safety procedures are checked against the listed sources, and current manufacturer instructions still take priority for the exact item in your shop.'] },
  ...
  editorial: {
    ...
    sections: [
      { heading: 'Original value is required', body: ['A page must add a buildable plan, measurement, calculation, test method, failure analysis, decision framework, or another concrete reader outcome. Rephrasing existing search results is not enough. A future draft stays out of the search index until it meets that standard.'] },
