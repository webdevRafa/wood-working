...
export function GuidePage() {
  ...
  return (
    <main>
      <header className="border-b border-walnut/10 bg-paper">
        <div className="mx-auto max-w-[1180px] px-5 pb-12 pt-8 sm:px-8 sm:pb-16 sm:pt-10">
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_300px] lg:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-pine px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-white">{guide.type}</span><span className={`text-[10px] font-black uppercase tracking-[0.16em] ${sourceBacked ? 'text-amber' : 'text-steel'}`}>{sourceBacked ? 'Source-backed guide' : 'Working research draft'}</span></div>
      ...
      <div className={`border-b ${sourceBacked ? 'border-pine/20 bg-[#eef5f0]' : 'border-[#b33a2b]/20 bg-[#fff4f1]'}`}><div className="mx-auto flex max-w-[1180px] items-start gap-3 px-5 py-4 text-sm leading-6 text-walnut sm:px-8">{sourceBacked ? <Check size={18} className="mt-0.5 shrink-0 text-pine" /> : <AlertTriangle size={18} className="mt-0.5 shrink-0 text-[#b33a2b]" />}<p>{sourceBacked ? <><strong>Evidence status:</strong> Central guidance was reviewed against the sources listed below. This is research-based editorial guidance; it does not claim hands-on testing.</> : <><strong>Working draft:</strong> This page remains accessible for transparent editorial review, but it is not included in the search index. Treat dimensions, loads, compatibility, and product details as unverified until this notice is replaced.</>}</p></div></div>
      ...
      <div className="mx-auto grid max-w-[1180px] gap-12 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[minmax(0,760px)_260px] lg:items-start">
        <article className="article-content min-w-0">
          ...
          {sourceBacked && guide.sources.length ? <section id="sources" className="scroll-mt-28 border-t border-walnut/10 py-9"><span className="text-[10px] font-black uppercase tracking-[0.18em] text-amber">Evidence</span><h2 className="mt-2 font-display text-3xl font-black leading-tight tracking-tight text-walnut sm:text-4xl">Sources and limits</h2><p className="mt-5 text-[17px] leading-8 text-charcoal/90">These sources support the general principles in this guide. The current manual, technical data sheet, safety data sheet, hardware drawing, or local requirement for the exact product and installation still takes priority.</p><ul className="mt-5 grid gap-3">{guide.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer" className="font-bold text-pine underline decoration-pine/30 underline-offset-4 hover:decoration-pine">{source.title}</a></li>)}</ul></section> : null}
        </article>

        <aside className="hidden lg:block lg:sticky lg:top-28"><div className="rounded-[1.2rem] border border-walnut/10 bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-steel">In this guide</p><nav className="mt-4 grid gap-3">{guide.sections.map((section) => <a key={section.id} href={`#${section.id}`} className="text-sm font-bold leading-5 text-walnut hover:text-pine">{section.heading}</a>)}</nav></div><div className="mt-4 rounded-[1.2rem] bg-sawdust p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-pine">Use this guide well</p><strong className="mt-2 block font-display text-lg text-walnut">Start with the real material</strong><p className="mt-2 text-xs leading-5 text-steel">Treat dimensions and prices as practical starting points. Confirm your stock, hardware, space, current product instructions, and local requirements before the irreversible step.</p></div></aside>
      </div>
