  ...
  return (
    <main>
      <header className="border-b border-walnut/10 bg-paper">
        <div className="mx-auto max-w-[1180px] px-5 pb-12 pt-8 sm:px-8 sm:pb-16 sm:pt-10">
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_300px] lg:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-pine px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-white">{guide.type}</span><span className="text-[10px] font-black uppercase tracking-[0.16em] text-amber">{guide.status === 'published' ? 'Published guide' : 'Editorial preview'}</span></div>
      ...
      <div className="mx-auto grid max-w-[1180px] gap-12 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[minmax(0,760px)_260px] lg:items-start">
        ...
        <aside className="hidden lg:block lg:sticky lg:top-28"><div className="rounded-[1.2rem] border border-walnut/10 bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-steel">In this guide</p><nav className="mt-4 grid gap-3">{guide.sections.map((section) => <a key={section.id} href={`#${section.id}`} className="text-sm font-bold leading-5 text-walnut hover:text-pine">{section.heading}</a>)}</nav></div><div className="mt-4 rounded-[1.2rem] bg-sawdust p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-pine">Evidence status</p><strong className="mt-2 block font-display text-lg text-walnut">{guide.status === 'published' ? 'Published and reviewed' : 'Editorial preview'}</strong><p className="mt-2 text-xs leading-5 text-steel">{guide.status === 'published' ? 'This guide has passed the current editorial review standard.' : 'This public preview is crawlable and remains visibly labeled while measurements, steps, safety notes, and evidence continue through editorial review.'}</p></div></aside>
      </div>
