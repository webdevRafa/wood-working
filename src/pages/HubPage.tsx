...
export function HubPage({ eyebrow, title, description, types, categoryIds }: HubPageProps) {
  ...
  const { guideIndex, loadingPublished } = useContent()
    ...
    <main>
      ...
      <section className="bg-paper py-12 sm:py-16">
        <div className="mx-auto max-w-[1280px] px-5 sm:px-8">
          ...
          <div className="mt-8 flex items-center justify-between"><p className="text-sm font-bold text-steel">{results.length} guide{results.length === 1 ? '' : 's'} in this workshop path</p><span className="text-[10px] font-black uppercase tracking-[0.15em] text-pine">{loadingPublished ? 'Checking the published library' : 'Published guides + editorial previews'}</span></div>
