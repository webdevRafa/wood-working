    ...
    <main>
      ...
      <section className="bg-paper py-12 sm:py-16">
        <div className="mx-auto max-w-[1280px] px-5 sm:px-8">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-steel">{results.length} guide{results.length === 1 ? '' : 's'} · {publishedCount} source-backed · {results.length - publishedCount} working draft{results.length - publishedCount === 1 ? '' : 's'}</p><span className="text-[10px] font-black uppercase tracking-[0.15em] text-pine">{loadingLibrary ? 'Opening the workshop library' : 'Published guidance + transparent previews'}</span></div>
