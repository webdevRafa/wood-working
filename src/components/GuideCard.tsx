    ...
    <article className={`group overflow-hidden rounded-[1.4rem] border border-walnut/10 bg-white transition hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(36,26,21,0.1)] ${compact ? '' : 'h-full'}`}>
      <Link to={guide.canonicalPath} className="block h-full">
        <div className={`relative overflow-hidden ${compact ? 'h-36' : 'h-48'} ${tones[Number(guide.id) % tones.length]}`}>
          <div className="absolute -right-5 -top-9 font-display text-[9rem] font-black leading-none text-walnut/[0.08]">{guide.id}</div>
          <div className="absolute inset-x-6 bottom-6 h-16 rotate-[-2deg] rounded-sm border-2 border-walnut/45 bg-paper/25 shadow-[6px_7px_0_rgba(36,26,21,0.12)]" />
        ...
        <div className={compact ? 'p-5' : 'p-6 sm:p-7'}>
          ...
          <div className="mt-6 flex items-center justify-between gap-4">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-pine"><ShieldCheck size={14} /> Editorial draft</span>
